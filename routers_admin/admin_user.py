...
def get_user_by_username(username: str, db: Session = Depends(get_db), admin: UserAuth = Depends(auth.get_current_user_admin)):
...
def promote_user_to_admin(id: int, db: Session = Depends(get_db), admin: UserAuth = Depends(auth.get_current_user_admin)):
    return db_user.promotr_to_admin(user_id = id, db = db, admin_id = admin.id)

@router.put('/promote_user_by_username/{username}')
def promote_user_to_admin_by_username(username: str, db: Session = Depends(get_db), admin: UserAuth = Depends(auth.get_current_user_admin)):
